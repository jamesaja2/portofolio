...
export default function WindowFrame({
  ...
  useEffect(() => {
    ...
    const clampToViewport = () => {
      setRect((current) => {
        ...
        const safeWidth = availableWidth > 0 ? availableWidth : initial.w
        const targetWidth = Math.max(220, Math.min(initial.w, safeWidth))
        const maxX = window.innerWidth - targetWidth - 16
        const boundedX = Math.max(16, Math.min(current.x, maxX >= 16 ? maxX : 16))
        return { ...current, w: targetWidth, x: boundedX }
      })
    }
  ...
  return (
    ...
    >
      ...
      >
        <div className={cn("truncate pr-2 text-xs sm:text-sm", habbo ? styles.title : "text-sm font-semibold")}>{title}</div>
